...
import LoadingComponet from "@/utils/DocUtils/Loading";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function AdminPage() {
  ...
  const filteredProjects = projectSearch
    ? projects?.filter((p) =>
        [p.name, p.client, p.by?.username, p.projectManager?.username].some(
          (field) => field?.toLowerCase().includes(projectSearch.toLowerCase())
        )
      )
    : projects;
  return (
    <div className="p-6 overflow-y-scroll hide-scrollbar">
      ...
      <section className="mb-4 flex items-center gap-4 overflow-y-scroll hide-scrollbar">
        <Link href={"/"} className="text-white bg-black px-4 py-2 rounded-lg">
          Back
        </Link>
        <Button
        ...
        {selectedTabs.userWithProjects !== true && (
          <Popover>
      ...
      <main className="overflow-x-auto">
        <h1 className="font-bold text-3xl py-2 px-0 underline text-blue-500 ">
          {selectedTabs.users === true && selectedTabs.projects === false
            ...
            : selectedTabs.projects
            ? "All Project's"
            : "All Project's with User's"}
        </h1>
        <div className="overflow-y-scroll max-h-[73vh]">
          <Table className="min-w-full border-collapse">
            <TableHeader className="sticky top-0 bg-white z-10 shadow">
              {selectedTabs.users === true &&
                selectedTabs.projects === false && (
                  <TableRow>
                    ...
                    <TableHead></TableHead>
                    <TableHead className="w-1 ml-2">DELETE</TableHead>
                  </TableRow>
                )}
              {selectedTabs.users === false &&
                selectedTabs.projects === true && (
                  <TableRow className="text-lg p-2 m-4">
                    ...
                    <TableHead>
                      No of Members
                      <span className="text-xs font-light p-1 mt-2">
                        (including self)
                      </span>
                    </TableHead>
              ...
              {selectedTabs.users === true &&
                ...
                filteredUsers?.map((user, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell
                      className=" w-[100px] flex justify-start items-center p-2 rounded-lg my-1 b-1 b-black ml-2"
                      onClick={() => {
                        setUpdateAnyUser(user);
                        setUpdateDialogOpen(true);
                      }}
                    >
                      <button className="flex flex-row font-medium gap-2 mt-1">
                        <Edit />
                      </button>
                    </TableCell>
                    <TableCell></TableCell>
                    <TableCell>
                      <AlertDialog>
                        <AlertDialogTrigger className="b-1 b-black w-[100px] flex justify-start items-center p-2 rounded-lg">
                          <button className="flex flex-row font-medium gap-2 py-1">
                            <Delete />
                          </button>
                        </AlertDialogTrigger>
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              ...
                              permanently delete your task and below related
                              tasks and remove your data from our servers.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
              ...
              {selectedTabs.users === false &&
                ...
                filteredProjects?.map((project, index) => (
                  <TableRow key={index} className="m-4">
                    ...
                    <TableCell>
                      {dayjs(project?.createdAt).format("DD/MM/YYYY, HH:mm:ss")}
                    </TableCell>
                    <TableCell className="">{project?.group?.length}</TableCell>
                    <TableCell
                      className="text-white bg-black  rounded-sm flex justify-center"
                      onClick={async () => {
                ...
                <div>
                  <div className="flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-3">
                      <Button
                        className="text-fuchsia-50 bg-amber-400 rounded-2xl hover:bg-amber-400"
