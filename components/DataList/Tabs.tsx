...
export default function CustomTabs({ data, identifier
    , userData
}: {
        ...
        return [
            {
                ...
                status: "Pending",
                createdAt: dayjs(new Date(data?.createdAt)).format("YYYY-MM-DD HH:mm:ss A"),
                children: [],
                features: {
                    documents: [],
                },
    ...
    const updateProjectAfterSubmitTaskStatus = async (newData: Response) => {                // newData = await newData.json();
        ...
        <div className="flex w-full p-5 flex-col">
            ...
            <Tabs defaultValue="users">
                <TabsList className="flex-1 px-2 py-2  w-full">
                    <TabsTrigger className="px-17 py-2.5" value="users">My Work</TabsTrigger>
                    {
                        (loginrole === "core-admin" || projectrole === "core-admin") && <TabsTrigger className="px-17 py-2.5 w-fit" value="tasks" >Tasks</TabsTrigger>
                    }
                </TabsList>
