...
export default function UsersTable({
  ...
  groupList: string[];
  setOnDelete: Dispatch<SetStateAction<number>>;
  ...
  const handleRemove = async (name: string) => {
    ...
    try {
      const request = await fetch(
        `/api/project/project-member-delete/${refreshData?._id}`,
        {
          method: "POST",
  ...
  return (
    <>
      <Table>
        <TableCaption>
          {(refreshData?.group?.length || 0) +
            " total member(s) & status is pending"}
        </TableCaption>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {loadingUsers ? (
            <div className="text-lg text-black font-medium flex justify-center w-full margin-auto padding-auto items-center">
              <span className="text-lg text-black font-medium w-full gap-2">
                <Loader2 className="animate-spin mr-1 mt-2" />
                <span>Please Wait Getted Group Members...</span>
              </span>
            </div>
          ) : Array.isArray(refreshData?.group) &&
            refreshData?.group?.length > 0 ? (
              ...
              if ("member" in user && "time" in user) {
                return (
                  <TableRow key={idx}>
                    ...
                    <TableCell>
                      {dayjs(user?.time).format("DD/MM/YY")}
                    </TableCell>
                    <TableCell>
                      <Button
