...
export function AppSidebar({
  ...
  ...props }: React.ComponentProps<typeof Sidebar> & {
    ...
    setClickProjectData: React.Dispatch<React.SetStateAction<singleProjectType | undefined>>,
    clickProjectData: singleProjectType | undefined,
    setOpenCreateProject: React.Dispatch<React.SetStateAction<boolean>>,
    user: {
      username: string, password: string, identifier: string, _id: string,
    }, setUser: React.Dispatch<React.SetStateAction<{
      username: string, password: string, identifier: string, _id: string,
    }>>
  }) {
  const [percentageScore, setPercentageScore] = React.useState<Map<string, number>>(new Map());
  const [projectData, setProjectData] = React.useState<singleProjectType[] | []>([]);
  const [activeItem, setActiveItem] = React.useState(data.navMain[0])
  const [filterProjects, setFilterProjects] = React.useState<string>("");
  const [selectedProjectChangeColor, setSelectedProjectChangeColor] = React.useState("");
  // hover Eyes for
  ...
  return (
    <>
      ...
      >
              ...
              <SidebarGroupContent>
                <SidebarMenu>
                  {data.navMain.map((item) => {
                    ...
                    return (
                      <SidebarMenuItem key={item.title} className="flex justify-center items-center p-[-10px]">
                        <SidebarMenuButton
                          tooltip={{
                            children: item.title,
                            hidden: false,
                        ...
                        {
                          item.title === "myWorkSpace" && <Link href={item.url}>
                            <Workflow size={20}/>
                          </Link>
                        }
                          {
                            item.title !== "admin" && item.title !== "myWorkSpace" && 
                            <item.icon className="ml-2" size={100} />
                          }
                          {
                            item.title === "admin" && loginrole === "core-admin" && <Link href={"/admin"}>
                              <PersonStandingIcon />
                            </Link>
                          }
                          {/* {
        ...
        <Sidebar collapsible="none" className="hidden flex-1 md:flex">
          <SidebarHeader className="gap-3.5 text-3xl border-b py-4">
            ...
            <SidebarInput className="h-9 mt-2 w-68" placeholder="Type to search..." onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
              e.preventDefault();
              ...
              <SidebarGroupContent>
                {
                  projectData?.map((item, index) => {
                    ...
                    if (filterProjects !== "" && !item.name.startsWith(filterProjects)) {
                      return;
                    }
                    return <a onMouseEnter={() => {
                      ...
                      href="#"
                      key={item?._id}
                      className={`border-b-1 first:border-t-1 first:border-t-gray-400 border-b-gray-400 hover:text-sidebar-accent-foreground flex flex-col items-start  pl-2 pt-2 pb-3 pr-2 text-sm leading-tight whitespace-nowrap ${selectedProjectChangeColor === item?.name ? "bg-gray-200" : "bg-wheat"} hover:bg-gray-300 ${hoverIndex === index ? 'h-[90px]' : 'h-[50px]'} `}
                    >
                      <div className="flex w-full items-center">
                        <span className="text-xl font-[400]">{NameLengthManage((item.name).substring(0, 1).toUpperCase() + (item.name)?.substring(1,).toLowerCase(), 20)}</span>{" "}
                        <span className="ml-auto text-sm font-semibold" onClick={() => {
                      ...
                      {
                        hoverIndex === index && <div className="w-[290px]">
                          <ProgressBar comeFrom={"app-sidebar"} percentage={
      ...
      {
        isOpenDetails && <FallBackShowProjectDialog clickProjectData={clickProjectData ?? null} isOpenShowProjectDetail={isOpenDetails} setIsOpenShowProjectDetailDialog={setIsOpenDetails} />
      }
      {
      ...
      <DialogContent>
        ...
        <div className="p-3 m-3 grid grid-cols-1">
          <div className="flex justify-start items-center m-2 gap-4.5">
            ...
            <Input value={details?.username} placeholder="Profile username" className="text-black" disabled />
          </div>
          <div className="flex justify-start items-center m-2 gap-4.5">
            <Label className="w-21">Role</Label>
            <Input value={data?.identifier} placeholder="Profile identifier" className="text-black" disabled />
          </div>
          {
            isUpdateClick && <>
              <div className="flex justify-start items-center m-2 gap-4.5">
                <Label className="w-17">oldPassword</Label>
                <Input value={details?.oldPassword} name="oldPassword" placeholder="Profile oldPassword" onChange={handleData} className="" />
              </div>
              {checkPassword && <div className="flex justify-start items-center m-2 gap-4.5">
                <Label className="w-17 mr-2">Enternew</Label>
                <Input value={details?.password} name="password" placeholder="password" onChange={handleData} className="" />
              </div>}
            </>
          }
          <div className="flex justify-start items-center gap-4 mt-5">
            {
              isUpdateClick && <Button onClick={() => {

              }} className="bg-black text-white font-medium">{loading ? <Loader className="animate-spin" /> : details?.password === '' ? <div onClick={() => {
                updatePassword()
              }}> Check </div> : <div className="flex flex-row gap-3" onClick={() => {
                update()
              }}>
                {
            ...
            {
              !isUpdateClick && <Button onClick={async () => {
